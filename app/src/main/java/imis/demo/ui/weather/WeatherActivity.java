...
import imis.demo.R;
import imis.demo.util.HttpUtil;
import imis.demo.util.LocalCity;
import imis.demo.util.Weather;
import imis.demo.util.isInterent;
...
//		locationplace = (TextView) findViewById(R.id.location);
	...
	public void GetFirstWeather(){
		...
		if (isInterent.hasInternet(this)) {
			...
			try {
				url1 = new URL(
						String.format(getString(R.string.weatherurl) + "?cityCode=%1$s&weatherType=0", LocalCity.getCityIdByName(City)));

				GetWeather nt = new GetWeather(url1, "0");
				nt.start();

				url2 = new URL(
						String.format(getString(R.string.weatherurl) + "?cityCode=%1$s&weatherType=1", LocalCity.getCityIdByName(City)));
